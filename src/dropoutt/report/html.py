...
<section>
  ...
  <div class="card" style="margin-bottom:var(--s-12)">
    ...
    within it are different subregions. <b>Reach</b> sums
    <code>min(1,&nbsp;density)</code> over those subregions: 1× is a full score,
    thinner coverage is a fraction, and over-representation does not count past one.</p>
    ...
    <table class="agrid">
      ...
      {% for a in s.atlas.grid %}
        <tr{{ ' class="empty"'|safe if not a.records }}>
          ...
          <td class="r reach"
              aria-label="{{ format_reach(a.effective_reach) }} of {{ a.cells|length }} subregions at map density; {{ a.cells|length - a.unreached }} hold any records">{% if a.cells %}{% if a.fully_reached %}<span class="reach-ok" aria-hidden="true">✓ </span>{% else %}<span class="reach-miss" aria-hidden="true">✕ </span>{% endif %}{% endif %}{{
            format_reach(a.effective_reach) if a.records else "0" }}/{{ a.cells|length }}</td>
  ...
  <div class="card" style="margin-top:var(--s-12)">
    ...
    <p class="t13 muted">The most unequal subregions: denser than the map means
    cut volume there; thinner means add more of that kind of record. Each line
    shows one of your own records from that cell.</p>
    <ul class="imbalances">
    {% for item in s.atlas.imbalances %}
      <li>
        ...
        <span>
          ...
          {% else %}<span class="yours muted">{{ "{:,}".format(item.records) }} records</span>{% endif %}
          <span class="cap">{% if item.area %}{{ item.area }}{% endif %} ·
            {{ "%.1f%%"|format(item.share * 100) }} of your placed records</span>
          <span class="act {{ item.action }}">{% if item.action == "cut" %}Cut — over-represented against the map{% else %}Grow — under-represented against the map{% endif %}</span>
        </span>
      </li>
    {% endfor %}
    </ul>
