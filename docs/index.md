...
with the data, but what it is about and what it is missing. It places records
on `atlas-v3`, the one frozen map that ships, and writes `atlas.html`,
`atlas.md` and `atlas.json`. The first run downloads
